-- Fix: Add user authorization check to prevent privilege escalation
-- The function should validate that the calling user matches p_user_id
...
DECLARE
  v_plan_id UUID;
BEGIN
  -- SECURITY: Validate that the calling user matches p_user_id
  -- This prevents privilege escalation where User A could create plans for User B
...
-- Also update the overloaded version without p_split_type
CREATE OR REPLACE FUNCTION public.generate_workout_plan_atomic(
  p_user_id uuid, 
  p_gym_id uuid, 
  p_goal_id text, 
